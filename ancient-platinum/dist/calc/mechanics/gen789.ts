...
export function calculateSMSSSV(
  ...
  if (move.named('Weather Ball')) {
    ...
    desc.moveType = type;
  }  else if (move.named('Ivy Cudgel') && attacker.item.includes('Mask')) {
  ...
  if (defender.curHP == defender.maxHP && defender.hasAbility('Tera Shell') && typeEffectiveness > 0) {
...
export function calculateDfModsSMSSSV(
  ...
  if (defender.hasAbility('Trash Shield') && typeEffectiveness < 1 && typeEffectiveness != 0) {
    dfMods.push(6144);
    desc.defenderAbility = defender.ability;
  }
