...
function getFinalSpeed(gen, pokemon, field, side) {
    ...
    if ((pokemon.hasAbility('Unburden') && pokemon.abilityOn) ||
        ...
        (pokemon.hasAbility('Backdraft') && ['Strong Winds', 'Windy'].includes(weather)) ||
        (pokemon.hasAbility('Swift Swim') && weather.includes('Rain')) ||
        (pokemon.hasAbility('Slush Rush') && ['Hail', 'Snow', 'Sleet'].includes(weather)) ||
        (pokemon.hasAbility('Surge Surfer') && terrain === 'Electric') ||
        (pokemon.hasAbility('Meadow Rush') && terrain === 'Grassy') ||
        (pokemon.hasAbility('Brain Blast') && terrain === 'Psychic') ||
        (pokemon.hasAbility('Star Sprint') && weather.includes('Starstorm')) ||
        (pokemon.hasAbility('Sludge Rush') && terrain === 'Poison') ||
        (pokemon.hasAbility('Toxic Rush') && weather.includes('Acid Rain')) ||
        (pokemon.hasAbility('Noctem Boost') && weather.includes('Eclipse'))) {
        speedMods.push(8192);
    }
