...
const Formats = [
  ...
  },
  {
    ...
    onValidateTeam(team) {
      ...
      for (const set of team) {
        ...
        const species = this.dex.species.get(set.species);
        const ability = this.dex.abilities.get(set.ability);
        if ((item.megaStone || item.isPrimalOrb) && item.itemUser?.includes(species.baseSpecies) || item.id === "ultranecroziumz" && species.name.startsWith("Necrozma-")) continue;
        if (this.ruleTable.isRestrictedSpecies(species) || this.ruleTable.isRestricted(`item:${item.id}`) || this.ruleTable.isRestricted(`ability:${ability.id}`)) {
          return [`${species.name} is not allowed to hold ${item.name}.`];
        }
        if (itemTable.has(item.id)) {
          return [
  ...
  },
  {
    ...
    mod: "gen9legendsou",
    ruleset: ["Standard", "Min Source Gen = 3", "Terastal Clause", "Dry Pass Clause"],
  ...
  // {
  // 	name: "[Gen 9] URL Draft League",
  // 	desc: `Specifically built for URL Draft League.`,
  // 	mod: 'gen9urldraftleague',
  // 	teraPreviewDefault: true,
  // 	ruleset: ['Standard', 'Terastal Clause', 'Min Source Gen = 3', 'Dry Pass Clause'],
  // 	banlist: ['Moody', 'Power Construct', 'Last Respects', 'Shed Tail'],
  // },
];
